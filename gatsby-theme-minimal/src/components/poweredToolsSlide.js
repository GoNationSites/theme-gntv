...
const PoweredToolsSlide = props => {
  ...
  const handleDurationChange = event => {
    setPoweredToolsConfig({ ...poweredToolsConfig, slideDuration: event.target.value })
  }
  ...
  const addSectionToTV = section => {
    setPoweredToolsConfig({ ...poweredToolsConfig, filteredOutSections: [...poweredToolsConfig.filteredOutSections, section.name] })
  }

  const removeSectionFromTV = (section) => {
    const newArr = poweredToolsConfig.filteredOutSections.filter(sec => sec !== section.name)
    setPoweredToolsConfig({ ...poweredToolsConfig, filteredOutSections: newArr })
  }

  const handleAdd = value => {
    setPoweredToolsConfig({ ...poweredToolsConfig, activeTypes: [...poweredToolsConfig.activeTypes, value] })
  }

  const handleRemoval = value => {
    const newValue = poweredToolsConfig.activeTypes.filter(item => item !== value)
  ...
  const handleFMToggle = (eventName) => {
    if (poweredToolsConfig.flyerModeEvents.includes(eventName)) {
      const replacedArr = poweredToolsConfig.flyerModeEvents.filter(evt => evt !== eventName)
      setPoweredToolsConfig({ ...poweredToolsConfig, flyerModeEvents: replacedArr })
    } else {
      setPoweredToolsConfig({ ...poweredToolsConfig, flyerModeEvents: [...poweredToolsConfig.flyerModeEvents, eventName] })
    }
  }
            ...
            <div className="column is-2 control flex-down">
              ...
              <label className="radio">
                <input
                  type="checkbox"
                  name="showTicker"
                  onChange={() => setPoweredToolsConfig({ ...poweredToolsConfig, showTicker: !poweredToolsConfig.showTicker })}
              ...
              <label className="radio">
                <input
                  type="checkbox"
                  name="hidePrices"
                  onChange={() => setPoweredToolsConfig({ ...poweredToolsConfig, showPrices: !poweredToolsConfig.showPrices })}
            ...
            <div className="column control full-width">
                  ...
                  <div className="select flex-down">
                    <label className="radio">
                      <input
                        ...
                        checked={poweredToolsConfig.displayType === "default"}
                      />
                ...
                <div className="flex-down control column">
                  ...
                  <div className="flex-down control">
                    ...
                    {props.albumNames.map((album, idx) => {
                      return (
                        <AlbumToggleCheckbox key={`${album}-${idx}`} toggleAlbum={toggleAlbum} idx={idx} album={album} />
                      )
                    })}
              ...
              <div className="flex-down control column evt-toggle">
                <div className="events-toggle-section">
                  <p>Select the event you'd like to only display. (Checking Flyer Mode removes all the content and just shows the image of the event)</p>

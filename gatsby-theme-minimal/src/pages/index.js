...
const IndexPage = (props) => {
  ...
  useEffect(() => {
    ...
    getData(gonationID, 'gallery').then(res => {
      setPhotoData(res.data.filter(el => el._id.includes('abm') && el.name !== 'Shouts'))
    })
    ...
    items
      .forEach((itm, idx) => {
        if (idx === 0) {
          ...
          if (items[idx - 1].sectionName === itm.sectionName && tmpArr.length < perPage) {
            tmpArr.push(itm)
          } else {
  ...
  useEffect(() => {
    if (
      (poweredToolsConfig.activeTypes.includes("item") || poweredToolsConfig.activeTypes.includes("section")) &&
      menuData.length
    ) {
      runMenu()
    }
    if (poweredToolsConfig.activeTypes.includes("event") && recurringData.length) {
      formatRecurringData()
    }
    if (poweredToolsConfig.activeTypes.includes("event") && eventData.length) {
      formatEventData()
    }
    if (shoutData.hasOwnProperty("shout") && poweredToolsConfig.activeTypes.includes("shout")) {
      formatShoutData()
    }
    if (photoData.length) {
      formatPhotoData()
    }
  }, [menuData, recurringData, eventData, shoutData, poweredToolsConfig.displayType, poweredToolsConfig.activeTypes, photoData])
  ...
  }, [isLoading, menuData, recurringData, shoutData, poweredToolsConfig.displayType, poweredToolsConfig.activeTypes, formattedPhotos])
  ...
  const handleFiltering = allSlideData => {
    if (poweredToolsConfig.eventItems.length) {
      return allSlideData.filter(slide => poweredToolsConfig.eventItems.includes(slide.name))
    } else {
      return allSlideData.filter(
        item =>
          ...
          poweredToolsConfig.activeTypes.includes(item.type) && !filteredOutAlbums.includes(item.albumName)
      )
    }
  }
      ...
      case "default":
        return handleFiltering(slideData).map((item, idx) => {
          return (
            <Slide
              key={`${item}-${idx}`}
              slideStyleType={item.type === "photo" || poweredToolsConfig.flyerModeEvents.includes(item.name) ? 'fullImageBG' : "random"}
              showcaseType={"default"}
              data={item}
              type={item.type}
              isTypeFlyer={poweredToolsConfig.flyerModeEvents.includes(item.name)}
        ...
        break
      case "list":
        return paginatedItems(10, sortFormattedMenu().filter(pile => !poweredToolsConfig.filteredOutSections.includes(pile.sectionName)))
          .map((pile, idx) => (
            <Slide
              key={`${pile}-${idx}`}
  ...
  return (
    <Layout>
      ...
      {shoutData.shout && poweredToolsConfig.showTicker ? <ShoutTicker shout={shoutData.shout} /> : ''}
